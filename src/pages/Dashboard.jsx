...
function Dashboard() {
  ...
  const pieData = CATEGORIES
    .map(cat => ({ name: cat, value: thisMonthExp.filter(e => e.category === cat).reduce((s, e) => s + e.amount, 0) }))
    .filter(d => d.value > 0)
    ...
    <div className="page-container">
      ...
      <div className="metric-row">
        <div className="metric-card">
          ...
          <div className="metric-value">RM {total.toFixed(2)}</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Transactions</div>
          <div className="metric-value">{thisMonthExp.length}</div>
        </div>
        <div className="metric-card">
          <div className="metric-label">Daily Avg</div>
          <div className="metric-value">RM {dailyAvg.toFixed(2)}</div>
        </div>
      </div>
            ...
            <div className="legend">
              {pieData.map(d => (
                <span key={d.name} className="legend-item">
                  <span className="legend-dot" style={{ background: COLORS[d.name] }}></span>
